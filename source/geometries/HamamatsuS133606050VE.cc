...
namespace nexus {
  ...
  const G4double HamamatsuS133606050VE::transverse_dim_      = 6.4*mm;
  const G4double HamamatsuS133606050VE::thickness_           = 1.35*mm;
  const G4double HamamatsuS133606050VE::window_thickness_    = 0.1*mm;
  const G4double HamamatsuS133606050VE::sensarea_td_         = 6.0*mm;
  const G4double HamamatsuS133606050VE::sensarea_thickness_  = 0.1*mm;
 
  //Define static methods
  G4double HamamatsuS133606050VE::GetWidth()     { return transverse_dim_; }
  G4double HamamatsuS133606050VE::GetThickness() { return thickness_; }
  G4double HamamatsuS133606050VE::GetHeight()    { return transverse_dim_; }

  void HamamatsuS133606050VE::Construct()
  {
    /// PHOTOSENSOR ENCASING

    G4String name = "HAMAMATSU_S13360_6050VE_MPPC";

    G4Box* encasing_solid =
      new G4Box(name, transverse_dim_/2., thickness_/2., transverse_dim_/2.);
    ...
    G4Box* aux_solid =
      new G4Box("AUX", transverse_dim_/2., aux/2., transverse_dim_/2.);

    name = "MPPC_SENSAREA";
    G4Box* sensarea_solid =
      new G4Box(name, sensarea_td_/2., sensarea_thickness_/2., sensarea_td_/2.);
    ...
    G4Box* window_solid =
      new G4Box(name, transverse_dim_/2., window_thickness_/2., transverse_dim_/2.);
    ...
    SensorSD* sensdet = dynamic_cast<SensorSD*>(SDMptr->FindSensitiveDetector("/PHOTOSENSOR/HS133606050VE"));
    if(!sensdet){
        sensdet = new SensorSD("/PHOTOSENSOR/HS133606050VE");
