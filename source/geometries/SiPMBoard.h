...
namespace nexus {
  ...
  public:
    ...
    G4double GetOverallHeight()      const;  ///< Returns the span of the geometry along the height axis (i.e. the sipm height if the sipm heighth is greater than the board height, and the board height otherwise)
    G4double GetBoardThickness()    const;  ///< Returns the thickness of the board alone
    G4double GetOverallThickness()  const;  ///< Returns the span of the geometry along the thickness axis (i.e. the sum of the thickness of the board plus the SiPM thickness)
  ...
  private:
    ...
    G4int base_id_;                     ///< ID of the first SiPM that is mounted. The i-th SiPM after the first one has an ID equal to base_id_+i
