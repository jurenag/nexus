...
#ifndef PTP_PHOTON_GENERATOR_H
#define PTP_PHOTON_GENERATOR_H
...
namespace nexus {
  ...
  class PTPPhotonGenerator: public G4VPrimaryGenerator
  {
  public:
    /// Constructor
    PTPPhotonGenerator();
  ...
  private:    
    ...
    std::piecewise_constant_distribution<G4double>* sampler_; ///< PTP emission spectrum histogram sampler

    void LoadNCheckPTPData();   ///< Loads PTP emission spectrum data into bin_edges_ and emission_spectrum_
    G4bool DataIsIllFormed();     ///< Checks whether the loaded data is ill-formed
    G4double RandomEnergy();    ///< Generate a random kinetic energy according to PTP emission spectrum.
  };
